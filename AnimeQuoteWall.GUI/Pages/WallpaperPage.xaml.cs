...
public partial class WallpaperPage : Page
{
    ...
    public WallpaperPage()
    {
        ...
        // Subscribe to size changes to auto-resize preview
    ...
    private void UpdateZoom()
    {
        ...
        // Reset image size constraints when zooming (let transform handle scaling)
        if (WallpaperPreview != null && Math.Abs(_currentZoom - 1.0) > 0.01)
        {
            WallpaperPreview.Width = double.NaN; // Auto size
            WallpaperPreview.Height = double.NaN; // Auto size
        }
        else
        {
            // Reset to fit when zoom is reset
            AdjustPreviewSize();
        }
    }
    ...
    /// <summary>
    /// Handles preview scroll viewer size changes to auto-resize the preview image.
    /// </summary>
    ...
    /// <summary>
    /// Adjusts the preview image size to fit within the available container space.
    /// Ensures the full preview is visible when not in fullscreen.
    /// </summary>
    private void AdjustPreviewSize()
    {
        try
        {
            if (WallpaperPreview?.Source == null || PreviewScrollViewer == null)
                return;

            var containerWidth = PreviewScrollViewer.ActualWidth;
            var containerHeight = PreviewScrollViewer.ActualHeight;

            if (containerWidth <= 0 || containerHeight <= 0)
                return;

            var bitmap = WallpaperPreview.Source as BitmapImage;
            ...
            // Apply the scale but don't override user zoom
            // Only adjust if current zoom is at default (1.0)
            if (Math.Abs(_currentZoom - 1.0) < 0.01)
            {
                // Set image size to fit container
                WallpaperPreview.Width = imageWidth * scale;
                WallpaperPreview.Height = imageHeight * scale;
            }
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"AdjustPreviewSize error: {ex.Message}");
        }
    }
}
