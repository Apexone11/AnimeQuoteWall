...
public partial class AnimationPage : Page
{
    ...
    /// <summary>
    /// Updates the animation preview with the first generated frame.
    /// </summary>
    private void UpdateAnimationPreview()
    {
        try
        {
            ...
            var firstFramePath = _generatedFrames[0];
            if (File.Exists(firstFramePath))
            {
                try
                {
                    ...
                    bitmap.UriSource = new Uri(firstFramePath, UriKind.Absolute);
                    bitmap.EndInit();
                    bitmap.Freeze();
