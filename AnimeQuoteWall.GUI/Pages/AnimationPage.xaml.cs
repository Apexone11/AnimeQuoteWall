...
public partial class AnimationPage : Page
{
    ...
    /// <summary>
    /// Timer for animating preview frames.
    /// </summary>
    private System.Windows.Threading.DispatcherTimer? _previewTimer;
    
    /// <summary>
    /// Current frame index for preview animation.
    /// </summary>
            ...
            Dispatcher.Invoke(() =>
            {
                try
                {
                    ...
                    else if (status.Result != null)
                    {
                        ...
                        {
                            ExportButton.IsEnabled = true;
                            ExportButton.Visibility = Visibility.Visible; // Ensure button is visible
                        }
                        if (GenerateAnimationButton != null)
                            GenerateAnimationButton.IsEnabled = true;
                        if (CancelButton != null)
                        ...
                        if (ProgressBorder != null)
                            ProgressBorder.Visibility = Visibility.Visible; // Keep progress bar visible
    ...
    /// <summary>
    /// Starts the preview animation by cycling through generated frames.
    /// </summary>
    private void StartPreviewAnimation()
    {
        try
        {
            ...
            var fps = int.TryParse(FpsTextBox.Text, out int f) ? f : 24;
    ...
    /// <summary>
    /// Handles preview timer tick to cycle through frames.
    /// </summary>
    private void PreviewTimer_Tick(object? sender, EventArgs e)
    {
        try
        {
            ...
            _currentPreviewFrameIndex = (_currentPreviewFrameIndex + 1) % _generatedFrames.Count;
            LoadPreviewFrame(_currentPreviewFrameIndex);
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"PreviewTimer_Tick error: {ex.Message}");
        }
    }

    /// <summary>
    /// Loads a specific frame into the preview.
    /// </summary>
    private void LoadPreviewFrame(int frameIndex)
    {
        try
        {
            if (_generatedFrames == null || frameIndex < 0 || frameIndex >= _generatedFrames.Count)
                return;
            ...
            AdjustAnimationPreviewSize();
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"LoadPreviewFrame error: {ex.Message}");
        }
    }

    /// <summary>
    /// Adjusts the animation preview size to fit within the container.
    /// </summary>
    private void AdjustAnimationPreviewSize()
    {
        try
        {
            if (AnimationPreview?.Source == null || AnimationPreviewScrollViewer == null)
                return;

            var containerWidth = AnimationPreviewScrollViewer.ActualWidth;
            var containerHeight = AnimationPreviewScrollViewer.ActualHeight;

            if (containerWidth <= 0 || containerHeight <= 0)
                return;

            var bitmap = AnimationPreview.Source as BitmapImage;
            ...
            // Set image size to fit container
            AnimationPreview.Width = imageWidth * scale;
            AnimationPreview.Height = imageHeight * scale;
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"AdjustAnimationPreviewSize error: {ex.Message}");
        }
    }
}
