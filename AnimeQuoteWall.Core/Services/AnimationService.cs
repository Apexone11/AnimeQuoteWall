...
public class AnimationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int frameIndex = 0; frameIndex < totalFrames; frameIndex++)
        {
            ...
            try
            {
                // Create wallpaper image for this frame
                bitmap = _wallpaperService.CreateWallpaperImage(backgroundPath, quote, settings);
                // Apply animation effects based on progress (fade, slide, etc.)
                ApplyAnimationEffects(bitmap, settings, profile, eased);
    ...
    private void ApplyAnimationEffects(System.Drawing.Bitmap bitmap, WallpaperSettings settings, AnimationProfile profile, float eased)
    {
        ...
        // - For "fade": fade-in text opacity using 'eased'
        // - For "slide": slide text left -> right using 'eased'
        // For now, keep simple (your current WallpaperService draws text); you can extend
        // that service later to accept a 'progress' parameter and vary position/opacity.

        // Start simple: do nothing now. Later, you'll pass 'eased' into DrawAnimatedQuote.
    }
}
